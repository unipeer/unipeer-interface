...
type RaiseDisputModalProp = {
  paymentAmount: string;
  paymentCurrency: string;
  sellerAddress: string;
  reasonForDispute: string;
  raiseDisputeCallback: MouseEventHandler<any>;
};

export function RaiseDisputeModal<RaiseDisputModalProp>({
  paymentAmount,
  paymentCurrency,
  sellerAddress,
  reasonForDispute,
  ...
  return (
    <div className="text-20 text-dark-dark-800">
      You are about to raise a dispute for an order via Kleros.{" "}
      <a
        href="#"
        className="w-full text-20 text-primary-color-500 text-20 font-normal underline font-paragraphs font-semibold cursor-pointer"
      >
        Learn more
      </a>
      <div className="flex flex-col mt-4 items-center justify-start w-auto h-auto rounded-lg bg-accent-2 py-6 px-4">
        <div className="flex flex-row items-center justify-start w-full text-20 text-dark-dark-500">
          Payment amount - {" "}
      ...
      <button
        type="submit"
        className="flex flex-row mt-8 items-center justify-center w-auto max-h-[56px] rounded-lg bg-state-alert py-8 gap-0"
        onClick={raiseDisputeCallback}
      >
